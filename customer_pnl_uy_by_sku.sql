...
,OUTPUT_DIF AS (
  ...
  LEFT JOIN (SELECT COUNTRY,CURRENCY_COPA,PERIOD_YEAR,SKU,BSP,BRAND,CATEGORY,FF,SUM(CM_ADJUSTED) COGS_MANUFACTURING,SUM(COGS_LOGISTIC) COGS_LOGISTIC,SUM(GROSS_SALES)GROSS_SALES,SUM(G2N)G2N,SUM(VOLUME_NET)VOLUME, SUM(NET_REVENUE)NET_REVENUE,SUM(GROSS_PROFIT)GROSS_PROFIT FROM  OUTPUT_3
