...
,OUTPUT_DIF AS (
  ...
  LEFT JOIN (SELECT COUNTRY,CURRENCY_COPA,PERIOD_YEAR,SKU,BSP,BRAND,CATEGORY,FF,SUM(CM_ADJUSTED) COGS_MANUFACTURING,SUM(COGS_LOGISTIC) COGS_LOGISTIC,SUM(GROSS_SALES)GROSS_SALES,SUM(G2N)G2N,SUM(VOLUME_NET)VOLUME FROM  OUTPUT_3
...
,OUTPUT_PIVOT AS (
  ...
  UNPIVOT( AMOUNT FOR ACCOUNT IN (GROSS_SALES,G2N,VOLUME,COGS_LOGISTIC,COGS_MANUFACTURING))
  --LIMIT 100
)
